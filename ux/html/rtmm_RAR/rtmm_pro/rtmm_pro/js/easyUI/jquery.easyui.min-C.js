...
$("<div class=\".my-pagination-info myPageSize\"></div>").appendTo(_84); //改变页面记录条数设置 3-3 个人添加的
ps.appendTo(".myPageSize");                                              //改变页面记录条数设置 3-3 个人添加的
...
}, beforePageText: "跳转到第", afterPageText: "页", displayMsg: "第 {from}-{to} 项，共 {total} 项&nbsp;&nbsp;|&nbsp;&nbsp;每页显示", nav: {
    first: {
...
if (item.children && item.children.length) {
    ...
    /**这段代码是在 tree-checkbox span 前插入 tree-icon tree-folder span
    *******************************************************************
    *<div class="tree-node" style="cursor: pointer;">
        ***<span class="tree-hit tree-collapsed"></span>         还未加载
        ***<span class="tree-icon tree-folder">                  还未加载
        ***</span><span class="tree-checkbox tree-checkbox0">    已经加载
        *</span><span class="tree-title">Languages</span>        已经加载
    *</div>
    ********************************************************************
    ****/
    if (item.state == "open") {
    ...
    /*1.*Checkbox start 这段代码是在 tree-title span 前插入 tree-checkbox span
    *******************************************************************
    *<div class="tree-node" style="cursor: pointer;">
        ***<span class="tree-hit tree-collapsed"></span>         还未加载
        ***<span class="tree-icon tree-iconXXXXX tree-folder">   还未加载
        ***</span><span class="tree-checkbox tree-checkbox0">    还未加载
        *</span><span class="tree-title">Languages</span>        已经加载
    *</div>
    ********************************************************************
    ****/
...
_101.push(node[0]);
    ...
    /*2.**********************tree-hit、tree-indent部分代码******************************/
if (item.children && item.children.length) {
    //$("<span class=\"tree-hit tree-collapsed\"></span>").prependTo(node);//这是原代码,这里导致第一节点默认为收缩，实际它却是打开的，所以导致父节点第一次点击有点问题;;;父节点都要打开，否则第一次点击有点儿问题
    $("<span class=\"tree-hit tree-expanded\"></span>").prependTo(node);
} else {
    if (item.state == "closed") {
        $("<span class=\"tree-hit tree-collapsed\"></span>").prependTo(node);
...
if (col.checkbox) {
    myff = "my-datagrid-cell-check";
} else {
    myff = "my-" + col.cellClass;
}
cc.push("<td class=\""+myff+"\" field=\"" + _670 + "\" " + cls + " " + _674 + ">");
if(col.checkbox){
var _674="";
}else{
var _674=_673;
