(function ($){
	...
	$.zWindow.open = function(options){
		var setting = $.extend({
			...
			top			:	false,
			left		:	false,
			windowType	: 	"iframe",	// iframe  html  text
			windowSrc	: 	"",
			beforeClose	:	false,
			afterClose	:	false,
			moveable	: 	true,
		...
		var models = {
			...
			iframe: '<iframe id="zwindow-frame{rnd}" name="zwindow-frame{rnd}" class="zwindow-iframe" frameborder="0" vspace="0" hspace="0" scrolling="no" webkitAllowFullScreen mozallowfullscreen allowFullScreen allowtransparency="true" ></iframe>'
		};
		//返回弹窗-window
		function getWindow() {
		...
		function resizeJDom(ZW, jDom, top, left, dimensionX, dimensionY){
			jDom.unbind("mousedown").bind("mousedown", function(e){
				var d = {
					...
					x: e.pageX,
					y: e.pageY,
					jResizeDomTop: [ZW.pannel.position().top],
					jResizeDomLeft: [ZW.pannel.position().left],
					jResizeDomsX:[ZW.pannel.width()],
					jResizeDomsY:[ZW.pannel.height()],
		...
		function createTitle(ZW){
			...
			ZW.titleBar = $(models.titleBar).height(40);
		...
		function createPannel(ZW){
			...
			//var selfWidth = setting.width;
			//var selfHeight = setting.height;
		    ...
		    if(setting.top){
		        ...
		        var top = (scrollTop + topHeight - setting.height) / 2;
		    	top = top<0?0:top;
		    ...
		    if(setting.left){
		        ...
		        var left = (topWidth + scrollLeft - setting.width) / 2;
		    	left = left<0?0:left;
		    	ZW.pannel.css("left", left);
		    	refreshSetting(ZW, {left: left});
		    }
		    
		    if (setting.footerBtn == true) {
		        ZW.content = $(models.content);
		        var str='<div class="Panel_footer"><span class="PanelBtn1">确定</span><span class="PanelBtn2">取消</span></div><div style="clear:both;"></div>';
		        ZW.content.append("<div style='height:20px;'>test</div>");
		    }
		    //if(setting.top){
			//	ZW.pannel.css("top", setting.top);
			//}
	...
	$.zWindow.close = function(data){
		...
		if(closeDom){
			...
			if( dataSetting.beforeClose!=undefined && (typeof dataSetting.beforeClose=="function")){
