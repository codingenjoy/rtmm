...
function initHypeMarketTree(){
	var count=0;
	var itemNo = $("#itemListFirstDivArea .tr_click_on")[0].cells[0].children[0].children[0].value;
	var supId=$('#crDcStoreNo').val();
	var catlgId=$('#crCatlgId').val();
	...
	$.post(ctx + "/rp/plan/getItemRegion?itemNo="+itemNo + '&catlgId=' + catlgId + '&supId=' + supId, {
	}, function(data) {
		$('#hypeMarketArea').tree({
			checkbox : true,
			data : data.tree,
			onCheck : function(node,checked) {
				count++;
				if(count>1){
					getStoreByRegion(node,checked,itemNo,catlgId,supId);
				}
			}
		});
	}, "json");
...
function getStoreByRegion(node,checked,itemNo,catlgId,supId){
	var itemStoreStr = "." + $("#itemStoreDivArea input[type='hidden']").map(function(){return $(this).val();}).get().join() + ".";
		...
		$.ajax({
			...
			success : function(data) {
				$.each(data,function(index,store){
					var disabled = "";
					if (itemStoreStr.indexOf(store.storeNO) >= 0) {
						disabled = "disabled = 'disabled'";
					}
					if(checked){
						$("#storeDiv").append("<div id='store_"+store.storeNO+"'><label>"
						+"<input type='checkbox' "+disabled+" class='check1' name='hypeMarketNos' onclick='checkChkAllBoxStat()' value="+store.storeNO+" storeName="+store.storeName+"></label>"
						+"<span>"+store.storeNO+"-"+store.storeName+"</span></div>");
					}else{
						$("#storeDiv").find("div[id=store_"+store.storeNO+"]").remove();
					}
				});
				if (checked) {
					$('#storeCheckAll').attr("checked", false);
				} else {
					allCheckedStatus();					
				}
			}
...
function getAllCheckedStores(){
	var storeVO = {};
	var dateStr = '${dateLong}';
	var storeArr=[];
	var storeArrObj=$("#storeDiv input[name='hypeMarketNos']:checked");
	if(storeArrObj.length>0){
		$.each(storeArrObj,function(index,storeElem){
			var storeObj = {};
			storeObj.storeNo = storeElem.value;
			storeObj.storeName = storeElem.getAttribute("storename");
			storeArr.push(storeObj);
		});
	}
	storeVO.storeArr = storeArr;
	storeVO.dateStr = dateStr;
	return storeVO;
}
//判断是否为全选状态
function allCheckedStatus() {
	var checkedSum = $("#storeDiv input[name='hypeMarketNos']:checked").length;
	var checkboxSum = $("#storeDiv input[name='hypeMarketNos']").length;
	if (checkedSum == checkboxSum && checkboxSum != 0) {
		$('#storeCheckAll').attr("checked", true);		
	} else {
		$('#storeCheckAll').attr("checked", false);
	}
}
