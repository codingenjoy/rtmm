...
<div class="htable_div">
	<table>
		<thead>
			<tr>
				<td><div class="t_cols" style="width: 250px;">姓名</div></td>
				<td><div class="t_cols" style="width: 250px;">用户名</div></td>
				<td><div class="t_cols" style="width: 150px;">状态</div></td>
				<td><div class="t_cols" style="width: 150px;">锁定状态</div></td>
	...
	<table class="single_tb w100">
		<c:if test="${not empty page.result }">
			<c:forEach items="${page.result}" var="item">
				<tr onclick="onClickRow('${item.staffId}','${item.locked}')">
					...
					<td style="width: 151px;">
						<c:choose>
							<c:when test="${item.locked==true}"><span style="color: red">已锁</span></c:when>
							<c:when test="${item.locked==false}">未锁</c:when>
						</c:choose>
					</td>
					<td style="width: 151px;">
