...
<script type="text/javascript">
  ...
  * 跳转
  */	
function pagination(/*int*/curPageNo,/*int,optional*/curPageSize) {
		var curPageObj;
		var pageNo;
		var pageSize;
		...
		pageSize = $(curPageObj).find("select#pageSize").val();
		var fpCallBack = $(curPageObj).find("#fpCallBack").val();
		var totalCount = $(curPageObj).find("#totalCount").val();
		...
		/*-------------------------------------------*/
		if(fpCallBack){
			rtmmCall(fpCallBack+'('+pageNo+','+pageSize+')');
		}else{
			pageQuery();
		}
...
{
	var inputPage = window.event.srcElement.value;
...
<div class="paging">
    <input type="hidden" id="fpCallBack" value="<c:if test='${not empty param.fpCallBack}'>${param.fpCallBack }</c:if>" />
	...
	<c:if test="${ not empty page.result}">
		<div class="fl_left">
			...
			<select name="pageSize" id="pageSize" onchange="pagination(${page.pageNo},this.value)">
				<option value="10"
		...
		<div class="fl_right page_list">
			...
			<c:if test="${page.pageNo < page.totalPages}">
				<a title="<auchan:i18n id="100002010"/>"
					onclick="javascript:pagination(${page.totalPages})"
					class="page_end_block"></a>
				<a title="<auchan:i18n id="100002006"/>"
					onclick="javascript:pagination(parseInt('${page.pageNo}')+1)"
			...
			<c:if test="${page.pageNo > 1}">
				...
				<a title="<auchan:i18n id="100002009"/>" onclick="javascript:pagination(1)"
