...
<script type="text/javascript">
  ...
  * 跳转
  */	
function pagination1(/*int*/curPageNo,/*int,optional*/curPageSize) {
	var curPageObj;
	var pageNo;
	var pageSize;
	var self = window.event.srcElement||window.target; 
 	while(true){
		if($(self).parent().hasClass("paging")){
			curPageObj = $(self).parent();
			break;
	    };
	    self = $(self).parent();
    } 
	pageSize = $(curPageObj).find("select#pageSize1").val();
	var totalCount = $(curPageObj).find("#totalCount1").val();
	pageNo  = curPageNo||1;
	pageSize = curPageSize || pageSize || 20;	
	if(!totalCount)totalCount=0;
	if((curPageNo-1)*pageSize>totalCount){
		pageNo=1;
	}
	$("#pageNo1").val(pageNo);
	if(curPageSize){
...
<div class="paging" id="paging1">
    <input type="hidden" id="totalCount1" value="${page.totalCount}"/>
	<c:if test="${ not empty page.result}">
		<div class="fl_left">
			...
			<select name="pageSize" id="pageSize1" onchange="pagination1(${page.pageNo})">
				<option value="10"
		...
		<div class="fl_right page_list">
			...
			<input value="跳转" type="button"
				onclick="jumpPage1(${page.totalPages})" />
			-->
			<!--末页和下一页  -->
			...
			<c:if test="${page.pageNo < page.totalPages}">
				<a title="<auchan:i18n id="100002010"/>"
					onclick="javascript:pagination1(${page.totalPages})"
					class="page_end_block"></a>
				<a title="<auchan:i18n id="100002006"/>"
					onclick="javascript:pagination1(parseInt('${page.pageNo}')+1)"
						...
						<c:choose>
							...
							<c:otherwise>
								<c:if test="${(6-v)<=page.totalPages}">
									<a onclick="pagination1(${(6-v)})">${(6-v)}</a>
								</c:if>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				...
				<c:otherwise>
					<c:choose>
						<c:when test="${page.pageNo<5}">
							<c:choose>
								<c:when test="${page.pageNo==1}">
									<%-- <a class="click" onclick="pagination1(${page.totalPages})">${page.totalPages}</a>&nbsp; 
									<a style="cursor: default;">...</a> --%>
									<a class="click" onclick="pagination1(5)">5</a>
									<a class="click" onclick="pagination1(4)">4</a>
									<a class="click" onclick="pagination1(3)">3</a>
									<a class="click" onclick="pagination1(2)">2</a>
								...
								<c:when test="${page.pageNo==2}">
									...
									<a class="click" onclick="pagination1(3)">3</a>
									<a class="click_block">${(page.pageNo)}</a>
									<a class="click" onclick="pagination1(1)">1</a>
								</c:when>
						...
						<c:when test="${page.pageNo>=5 and (page.pageNo+5)<=page.totalPages}">
							<%-- <a class="click" onclick="pagination1(${page.totalPages})">${page.totalPages}</a>&nbsp; 
							<a style="cursor: default;">...</a> --%>
							<a class="click" onclick="pagination1(${(page.pageNo+2)})">${(page.pageNo+2)}</a>
							<a class="click" onclick="pagination1(${(page.pageNo+1)})">${(page.pageNo+1)}</a>
							<a class="click_block">${(page.pageNo)}</a>
							<a class="click" onclick="pagination1(${(page.pageNo-1)})">${(page.pageNo-1)}</a>
							<a class="click" onclick="pagination1(${(page.pageNo-2)})">${(page.pageNo-2)}</a>
							<!-- <a style="cursor: default;">...</a>
						...
						<c:otherwise>
							<c:choose>
								<c:when test="${page.pageNo==page.totalPages}">
									...
									<a class="click" onclick="pagination1(${(page.pageNo-1)})">${(page.pageNo-1)}</a>
									<a class="click" onclick="pagination1(${(page.pageNo-2)})">${(page.pageNo-2)}</a>
									<a class="click" onclick="pagination1(${(page.pageNo-3)})">${(page.pageNo-3)}</a>
									<a class="click" onclick="pagination1(${(page.pageNo-4)})">${(page.pageNo-4)}</a>
			...
			<c:if test="${page.pageNo > 1}">
				...
				<a title="<auchan:i18n id="100002009"/>" onclick="javascript:pagination1(1)"
