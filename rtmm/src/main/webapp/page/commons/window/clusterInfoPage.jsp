...
$('.cluster_tr').die().live("dblclick",function(){
	selectCluster($(this));
});
...
function selectCluster(obj){
	if(obj.length == 1)
	{
		...
		var clusterbatchPriceChngInd = $(obj).attr('clusterbatchPriceChngInd');
	 	...
	 	$('#clstrdisplay').attr('value',clusterid + '-' + clusternm);
			...
			<table>
             <thead>
                 <tr>
					...
					<td><div class="t_cols" style="width:145px;">商品同价</div></td>
	 	...
	 	<div class="btable_div" style="height:280px;">
           <table  class="single_tb w100"><!--multi_tb为多选 width:1001px;-->
			<c:forEach items="${page.result }" var="item_cluster">
				<tr class='cluster_tr' clusterId="${item_cluster.clstrId}" clusterName="${item_cluster.clstrName}" clusterType="${item_cluster.clstrType}" clusterbatchPriceChngInd="${item_cluster.batchPriceChngInd}">
					...
					<td align="center" style="width:145px;padding-left:5px;"><auchan:getDictValue code="${item_cluster.clstrType}" mdgroup="ITEM_CLUSTER_CLSTR_TYPE"></auchan:getDictValue></td>
					<td align="center" style="width:145px;padding-left:5px;"><auchan:getDictValue code="${item_cluster.batchPriceChngInd}" mdgroup="ITEM_CLUSTER_BATCH_PRICE_CHNG_IND"></auchan:getDictValue></td>					
...
<jsp:include page="/page/commons/pageSet.jsp"></jsp:include>
	<div class="PanelBtn">
		<div onclick="selectCluster($('tr.btable_checked'))" class="PanelBtn1">确定</div>
