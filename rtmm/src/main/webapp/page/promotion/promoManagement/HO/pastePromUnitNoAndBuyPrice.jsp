...
function addPromUnitNoAndBuyPriceList(){
	...
	if(!inputListStr){
		top.jAlert("warning","请输入代号和促销进价","提示信息");
		return false;
	}
   ...
   //circulate all lines and get the valid lines and the error lines.
	$.each(lines, function(index, value){
		if ($.trim(value)){
			...
			if (!isNumber(unitNoVal,8) || !isFloat(promBuyPriceVal,5,4)){
				if(isNumber(unitNoVal,8)&&isFloat(promBuyPriceVal,5,99)){
					var dotIndex=promBuyPriceVal.indexOf(".");
					promBuyPriceVal=promBuyPriceVal.substring(0,dotIndex+5);					
				}else{
					errorMsg += value+'\n';
					return true;
				}
	...
	if(validItems.length>50){
		top.jAlert("warning","代号和促销进价最多输入50组","提示信息");
		return null;
	}
	if(errorMsg){
		$('#inputList').val("");
	...
	<div class="p51_panel">
		<div>
			<textarea id="inputList" name="inputList" class="w90 txtarea"
				rows="3" placeholder='请输入代号和促销进价,格式"货号 促销进价"'></textarea>
