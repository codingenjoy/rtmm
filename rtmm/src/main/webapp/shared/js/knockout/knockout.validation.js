...
kv.rules['webSite'] = {
		validator: function (webAddress, validate) {
			if (!validate) { return true; }
			if (kv.utils.isEmptyVal(webAddress)) { return true; } // makes it optional, use 'required' rule if it should be required
			if (typeof (webAddress) !== 'string') { return false; }
			webAddress = webAddress.replace(/\s+|\s+$/g, "");
			return validate && webAddress.match(/^((http|ftp|https):\/\/)?w{3}\.[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?/);
		},
		message: 'Please specify a valid webSite number'
	};
kv.rules['dateCompare'] = {
		validator: function (endDate, startDate) {
			...
			return startDateTime >= endDateTime ? false:true;
		},
		message: 'Please endDate should after startDate.'
	};
