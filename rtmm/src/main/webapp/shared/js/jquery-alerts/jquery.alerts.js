...
//
// Usage:
//		jAlert( alertType, message, [title, callback] ),top.jAlert(alertType, 'ddd','测试',function(){alert('测试回调')})
//              alertType有4个值error, success, warning, common
//		jConfirm( message, [title, callback] ),top.jConfirm('ddd','测试',function(d){alert('测试回调'+d)})
//		jPrompt( message, [value, title, callback] ),top.jConfirm('ddd','测试',function(d){alert('测试回调'+d)})
// 
// History:
//
...
(function($) {
	...
	$.alerts = {
		...
		alertButton: 'OK',   // text for the alertbutton
	...
	jErrorAlert = function (message, callback, title, opts) {
	    if (jQuery.isFunction(callback)) {
	        title = title == undefined ? '提示消息' : title;
	    } else {
	        title = callback == undefined ? '提示消息' :title = callback;
	        callback = undefined;
	    }
	    
	    jAlert("error", message, title, callback, opts);
	};
	
	jSuccessAlert = function (message, callback, title, opts) {
	    if (jQuery.isFunction(callback)) {
	        title = title == undefined ? '提示消息' : title;
	    } else {
	        title = callback == undefined ? '提示消息' :title = callback;
	    ...
	    jAlert("success", message, title, callback, opts);
	};
	
	jWarningAlert = function (message, callback, title, opts) {
	    if (jQuery.isFunction(callback)) {
	        title = title == undefined ? '提示消息' : title;
	    } else {
	        title = callback == undefined ? '提示消息' :title = callback;
	    ...
	    jAlert("warning", message, title, callback, opts);
	};
	
	jCommonAlert = function (message, callback, title, opts) {
	    if (jQuery.isFunction(callback)) {
	        title = title == undefined ? '提示消息' : title;
	    } else {
	        title = callback == undefined ? '提示消息' :title = callback;
	    ...
	    jAlert("common", message, title, callback, opts);
	};
