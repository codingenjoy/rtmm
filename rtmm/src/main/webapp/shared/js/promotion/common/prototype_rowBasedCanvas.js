...
RowBasedCanvas.prototype.show = function(/*Array*/pRowDataArrayIn/*and more variants*/){
	...
	for (var dataIndex=0;dataIndex<pRowDataArrayIn.length;dataIndex++){
		...
		if (!oneNewRowNode){
			var oneNewRowNode = document.createElement(this._rowTagNameOnCreate);
			containerNode.appendChild(oneNewRowNode);
			oneNewRowNode.innerHTML = newRowTemplateHTMLStr;
		}
		//回调 由子类实现的"执行自定义逻辑"函数;
		this.onAfterOneNewRowCreated(oneNewRowNode, moreVariantArgs);
...
//取当前被选中[ROW]的唯一识别码（一般为业务PK）
RowBasedCanvas.prototype.getActivatedRowPK = function(/*None*/){
	return this.rowSelectedTag.pk;
};
//取当前被选中[ROW]的索引值
RowBasedCanvas.prototype.getActivatedRowIndex = function(/*None*/){
 ...
 * 父类缺省为NULL实现，具体的逻辑由子类完成.
 */
RowBasedCanvas.prototype.setRowMemberEditableByNode = function(/*Node*/rowNodeIn, /*int*/rowMemberIdxsIn){
	//0.检测传入的ROW级元素是否被包含在当前的画布中；
	var contained = this.containsNode(rowNodeIn);
	if (!contained) return;
	//1.获取“嵌套索引”
	var idxArr = this.getTruncatedArray(arguments, 1);
	//2.根据嵌套索引，抓取目标元素
	var thMemberElem = this._getNestedRowMemberNodeByIndexArray(rowNodeIn, idxArr);
	this.setElementEditable(thMemberElem, true);
	this.onAfterAnyMemberEnterEditable(thMemberElem);
};
...
RowBasedCanvas.prototype.setRowMemberReadonlyByNode = function(/*Node*/rowNodeIn /*,variant*/){
	//0.检测传入的ROW级元素是否被包含在当前的画布中；
	var contained = this.containsNode(rowNodeIn);
	if (!contained) return;
	//1.获取“嵌套索引”
...
RowBasedCanvas.prototype.setElementEditable = function(/*Node*/anyNodeIn,/*boolean*/editable){
	if (editable){
		...
		if(anyNodeIn.nodeName=="SELECT"){
			$(anyNodeIn).find("option[value !='"+anyNodeIn.value+"']").each(function(){
				$(this).remove();
			});
		}
	}
...
RowBasedCanvas.prototype.getRowMemberNodeByIndex = function(/*int*/rowIndexIn, /*int*/memberIndexIn){
	...
	var nodeIdxArr = this.getTruncatedArray(arguments, 1);
	var thMemberNode = this._getNestedRowMemberNodeByIndexArray(rowNode, nodeIdxArr);
	return thMemberNode;
};
...
/*
 * 对当前画布上所有行级元素的[指定位置]上的子元素值进行更新。
 */
RowBasedCanvas.prototype.setAllRowsMemberNodeValue = function(/*String*/newNodeValIn/*,更多的可选参数int*/){
	var moreIndexs = this.getTruncatedArray(arguments, 1);
	var containerNode = this.getContainerNode();
	for (var rowIdx=0;rowIdx<containerNode.children.length;rowIdx++){
		var oneRowNode = containerNode.children[rowIdx];
		var memberNode = this._getNestedRowMemberNodeByIndexArray(oneRowNode, moreIndexs);
		memberNode.value = newNodeValIn || '';
	}
};
/*
 * 在编辑态时，设置[目标元素]的样式为"正常态|错误态".
 */
RowBasedCanvas.prototype.setRowMemberErrorInput = function(/*Node*/anyRowMemberNodeIn,/*String*/errTooltip,/*int*/rowMemberIndexIn/*,更多的可选参数int*/){
	var currRowElem = this.getRowNodeFromAnyElement(anyRowMemberNodeIn);
	var moreIndexs = this.getTruncatedArray(arguments, 2);
	var memberNode = this._getNestedRowMemberNodeByIndexArray(currRowElem, moreIndexs);
	if (!memberNode) return;
	$(memberNode).addClass('errorInput');
	$(memberNode).attr("title", errTooltip);
};
/*
 * 在编辑态时，设置[目标元素]的样式为"正常态|错误态".
 */
RowBasedCanvas.prototype.clearRowMemberErrorInput = function(/*Node*/anyRowMemberNodeIn,/*int*/rowMemberIndexIn/*,更多的可选参数int*/){
	var currRowElem = this.getRowNodeFromAnyElement(anyRowMemberNodeIn);
	var moreIndexs = this.getTruncatedArray(arguments, 1);
	var memberNode = this._getNestedRowMemberNodeByIndexArray(currRowElem, moreIndexs);
	if (!memberNode) return;
	$(memberNode).removeClass('errorInput');
	$(memberNode).attr("title", '');
};

/*
 * 清理当前画布元素.
 */
RowBasedCanvas.prototype.destroy = function(/*none*/){
