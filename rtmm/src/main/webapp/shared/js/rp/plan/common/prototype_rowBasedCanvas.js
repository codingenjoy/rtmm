...
RowBasedCanvas.prototype.clickRowByIndex = function(/*int*/rowIndexIn){
	var containerNode = this.getContainerNode();
	var maxRowIndex = this.getRowCount() - 1;
...
RowBasedCanvas.prototype._getNestedRowMemberNodeByIndex = function(/*Node*/rowNodeIn /*,more ints*/){
	...
	for (var argIdx=0;argIdx<moreArgsIndexes.length;argIdx++){
		if (!pseudoTargetElem) break;
		var nestedIdx = moreArgsIndexes[argIdx];
		pseudoTargetElem = pseudoTargetElem.children[nestedIdx];
	}
...
//错误信息标红
RowBasedCanvas.prototype.errorElementFlag = function(/*Node*/nodeElement){
	var nodeValue = nodeElement.value;
	if ($.trim(nodeValue) == "") {
		$(nodeElement).addClass("errorInput");
	}
};
//提示信息
RowBasedCanvas.prototype.hintMessage = function(/*Node*/nodeElement, /*String*/message){
	$(nodeElement).attr("title", message);
};