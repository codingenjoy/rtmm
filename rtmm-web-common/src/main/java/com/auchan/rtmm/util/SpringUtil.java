...
//		System.out.println(getStringLength("12 345 abcd().,/?-_=+ 哈哈我们"));
	 ...
	 * 得到字符串的前 N 个字, 加: ...
	 */
	public static String getPartStringAddThreePoint(String str, int maxLength) {
		return (hasText(str) && str.length() > maxLength) ? str.substring(0, maxLength) + "...": str;
	}
	
	public static void main2345(String[] args) {
		System.out.println(doWithDoubleNumber(-1.0));
		System.out.println(doWithDoubleNumber(0));
		System.out.println(doWithDoubleNumber(0.00000001));
		System.out.println(doWithDoubleNumber(0.001));
		System.out.println(doWithDoubleNumber(0.01));
		
		System.out.println(doWithDoubleNumber(123));
		System.out.println(doWithDoubleNumber(1234));
		System.out.println(doWithDoubleNumber(12345));
		System.out.println(doWithDoubleNumber(12345.1));
		System.out.println(doWithDoubleNumber(12345.129));
		System.out.println(doWithDoubleNumber("0.00"));
		System.out.println(doWithDoubleNumber("8888111111.00"));
	}
	 ...
	 * 12345 to 12,345.00
	 * 
	 * 12345.2 to 12,345.20
	 * 
	 * 12345.6789 to 12,345.68
	 * 
	 * 临时方法，待改进  ... TODO
	 */
	public static String doWithDoubleNumber(double doubleNum) {
		if(doubleNum < 0.00001) {
			return "0.00";
		}
		
		doubleNum = Arith.math45(doubleNum);
		
		String str = String.valueOf(doubleNum); //1234.56
		int index = str.indexOf(".");
		
		if(index <= 0) { //整数
			return doWithNumber(NumberUtil.parseLong(str)) + ".00";
		} else { //小数
			String zheng = str.substring(0, index);
			String xiao = str.substring(index + 1, str.length());
			
			if(xiao.length() == 1) {
				return doWithNumber(NumberUtil.parseLong(zheng)) + "." + xiao + "0";
			} else {
				return doWithNumber(NumberUtil.parseLong(zheng)) + "." + xiao.substring(0, 2);
			}
		}
	}
	 ...
	 * 临时方法，待改进  ... TODO
	 */
	public static String doWithNumber(Long longNum) {
		long num = NumberUtil.parseLong(longNum);
		...
		if(num <= 0) {
			...
			return str.substring(0, 1) + "," + str.substring(1, len);
		} else if(num <= 99999L) {
			return str.substring(0, 2) + "," + str.substring(2, len);
		} else if(num <= 999999L) {
			return str.substring(0, 3) + "," + str.substring(3, len);
		} else if(num <= 9999999L) { // 9,999,999
			return str.substring(0, 1) + "," + str.substring(1, 4) + "," + str.substring(4, len);
		} else if(num <= 99999999L) { // 99,999,999
			return str.substring(0, 2) + "," + str.substring(2, 5) + "," + str.substring(5, len);
		} else if(num <= 999999999L) { // 999,999,999
			return str.substring(0, 3) + "," + str.substring(3, 6) + "," + str.substring(6, len);
		} else if(num<= 9999999999L){
			return str.substring(0, 3) + "," + str.substring(3, 6) + "," + str.substring(6, 9) + "," +str.substring(9, len);
		}
...
//		}
	...
	/**
	 * 说明：这个方法从 Hibernate 中 ImprovedNamingStrategy 拷贝过来。在 Hibernate 的 ImprovedNamingStrategy 类中 ，它是 protected 方法
	 * 
	 * 作用和上面的 underscoreName 方法类似，
	 * 在一般情况下，返回结果 等同 上面的 underscoreName 方法
	 */
